...
def load_entries(ws_entries) -> pd.DataFrame:
    ...
    for col in ["Start","End"]:
        if col in df:
            df[col] = df[col].apply(to_time_cell)
        else:
            df[col] = None
...
with tab_entries:
    ...
    for idx, row in enumerate(st.session_state.intervals_v5):
        ...
        st_start_str = c[2].text_input("Start Time (e.g. 730, 7:30, 5pm)", value=row.get("start_time",""), key=key_st)
        st_end_str = c[4].text_input("End Time (e.g. 1700, 5pm)", value=row.get("end_time",""), key=key_et)
    ...
    for row in st.session_state.intervals_v5:
        ...
        if edate == sdate and end_dt <= start_dt:
            end_dt = end_dt + dt.timedelta(days=1)
                # Removed 24h hard cap; long intervals will be split across days.
