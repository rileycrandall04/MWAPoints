...
def _minute_multiplier(minute_of_day: int, weekend_or_holiday: bool) -> float:
    """Return the availability multiplier based on policy bands & weekend/holiday rules."""
    ...
    if weekend_or_holiday:
        ...
        # Weekday: base 07–17; +10% 17–23; +25% 23–07
        if 7 <= hour < 17:
            return 1.00
        elif 17 <= hour < 23:
            return 1.10
...
def _minute_rate_pts(category: str, m: int, wknd_hol: bool) -> float:
    ...
    if category == "Unrestricted Call":
        return 3.5
    return 0.0  # Cardiac coverage handled at day level

def _split_interval_to_day(start_t: dt.time, end_t: dt.time):
    """Return list of (start_min, end_min) within 0..1440 for same calendar day.
       If overnight, only returns up to midnight; the following day must be stored as a separate row.
    """
...
def compute_day_time_points(date_obj: dt.date, df_entries: pd.DataFrame):
    ...
    # If any row explicitly marks Holiday=True, treat the whole date as holiday
    is_holiday = False
    if "Holiday" in df_entries.columns:
        is_holiday = bool(df_entries["Holiday"].fillna(False).astype(bool).any())

    weekend_or_holiday = wknd or is_holiday
    ...
    for _, r in df_entries.iterrows():
        ...
        for (smin, emin) in _split_interval_to_day(stime, etime):
            for m in range(smin, emin):
                rate = _minute_rate_pts(cat, m, weekend_or_holiday)
    ...
    for m, cat in enumerate(winner_cat):
        ...
        p = _minute_rate_pts(cat, m, weekend_or_holiday) / 60.0
        total_pts += p
...
def compute_entry_time_points(row, date_is_weekend_or_holiday: bool):
    """Straight hourly/band math WITHOUT dominance (used only for per-entry display)."""
    # Rate bands split
    ...
    mins = []
    if emin > smin:
        mins = range(smin, emin)
    elif emin < smin:
        mins = list(range(smin, 1440))
    else:
        mins = []
    pts = 0.0
    for m in mins:
        pts += _minute_rate_pts(row["Category"], m, date_is_weekend_or_holiday) / 60.0
    # Min 80 for Assigned entries is applied at the DAY level only (to avoid double counting across overlaps)
    return round(pts, 2)
...
def ensure_user_sheet(gc):
    ...
    # Entries with Holiday column
    try:
        ws_entries = sh.worksheet("Entries")
    except Exception:
        ws_entries = sh.add_worksheet(title="Entries", rows=2000, cols=20)
...
def write_month_sheets(sh, df_entries):
    ...
    # Compute entry totals WITHOUT dominance (dominance is displayed in daily totals)
    dfe["Entry Base Time Points"] = dfe.apply(
        lambda r: compute_entry_time_points(r, (r["Holiday"] or (r["Date"].weekday()>=5))), axis=1
    )
    ...
    dfe["Entry Total Points"] = dfe["Entry Base Time Points"] + dfe["Prod"] + dfe["Extra"] + (dfe["TEE Exams"].fillna(0.0).astype(float) * 22.0)
...
def write_monthly_summary(sh, df_entries):
    ...
    # Build per-day totals using dominance to avoid overlap double count
    per_day = []
...
with tab_entries:
    ...
    cc_head = st.columns([1,1,1.6,1,1])
    date = cc_head[0].date_input("Date", value=dt.date.today(), format="MM/DD/YYYY")
    is_holiday = cc_head[1].checkbox("Holiday", value=False, help="Applies weekend/holiday differentials for this date.")
    tee = cc_head[2].number_input("TEE Exams (22 pts each)", min_value=0, step=1, value=0)
    prod = cc_head[3].number_input("Productivity Points", min_value=0.0, step=1.0, value=0.0)
    extra = cc_head[4].number_input("Extra Points", min_value=0.0, step=1.0, value=0.0)
    notes = st.text_input("Notes", "")

    st.caption("Times are logged to the minute. Overlaps are resolved by counting only the higher-paying category per minute.")

    # Multi-interval widget
    ...
    for i, row in enumerate(st.session_state.intervals):
        ...
        c = st.columns([1.8,1,1,0.3])
        cat = c[0].selectbox("Category", CATEGORIES, index=CATEGORIES.index(row["category"]) if row["category"] in CATEGORIES else 0, key=f"cat_{i}")
        stext = c[1].text_input("Start (e.g. 730, 7:30, 5pm)", row.get("start",""), key=f"st_{i}")
        etext = c[2].text_input("End (e.g. 1700, 5:00pm)", row.get("end",""), key=f"et_{i}")
        delete = c[3].button("🗑️", key=f"del_{i}")
        if not delete:
            new_intervals.append({"category": cat, "start": stext, "end": etext})
    st.session_state.intervals = new_intervals

    # Build preview rows for THIS date (split overnight at midnight into next day row)
    preview_list = []
    for row in st.session_state.intervals:
        stime = parse_time_any(row["start"])
        etime = parse_time_any(row["end"])
        if not (stime and etime):
            continue
        if etime < stime:
            # Split across midnight
            preview_list.append({"Date": date, "Holiday": is_holiday, "Category": row["category"], "Start": stime, "End": dt.time(23,59),
                                 "TEE Exams": 0, "Productivity Points": 0.0, "Extra Points": 0.0, "Notes": notes})
            preview_list.append({"Date": date + dt.timedelta(days=1), "Holiday": is_holiday, "Category": row["category"], "Start": dt.time(0,0), "End": etime,
                                 "TEE Exams": 0, "Productivity Points": 0.0, "Extra Points": 0.0, "Notes": f"(overnight from {date}) {notes}".strip()})
        else:
            preview_list.append({"Date": date, "Holiday": is_holiday, "Category": row["category"], "Start": stime, "End": etime,
                                 "TEE Exams": 0, "Productivity Points": 0.0, "Extra Points": 0.0, "Notes": notes})

    preview_df = pd.DataFrame(preview_list)

    # Dominance-based time points for the main (first) date preview only
    tpts = 0.0
    assigned_min = False
    if not preview_df.empty:
        same_day_df = preview_df[preview_df["Date"] == date]
        if not same_day_df.empty:
            tpts, per_cat_min, assigned_min = compute_day_time_points(date, same_day_df)
    ...
    if not preview_df.empty:
        ...
        if assigned_min:
            st.info("Assigned minimum 80 pts applied in dominance computation for this date.")
        st.markdown(f"**New time points (dominance applied for {date.strftime('%m/%d/%Y')}):** {tpts:.2f}")
        st.markdown(f"**New productivity points:** {float(prod):.2f}")
        if float(extra or 0.0) != 0.0:
            st.markdown(f"**New extra points:** {float(extra):.2f}")
        if int(tee or 0) > 0:
            st.markdown(f"**New TEE points:** {int(tee)*22:.2f}")
    else:
        st.caption("Add at least one valid interval to see preview.")

    # Current day existing totals (from sheet)
    ...
    projected_total = current_daily_total + tpts + float(prod) + float(extra or 0.0) + float(int(tee or 0)*22.0)
    st.markdown(f"**Projected total for {date.strftime('%m/%d/%Y')}: {projected_total:.2f} points**")

    # Save
    if st.button("Add to Sheet"):
        if not preview_df.empty:
            # Attach TEE/Prod/Extra to the FIRST row only (so they're counted once)
            preview_df.loc[:, "TEE Exams"] = 0
            preview_df.loc[:, "Productivity Points"] = 0.0
            preview_df.loc[:, "Extra Points"] = 0.0
            preview_df.loc[preview_df.index[0], "TEE Exams"] = int(tee or 0)
            preview_df.loc[preview_df.index[0], "Productivity Points"] = float(prod or 0.0)
            preview_df.loc[preview_df.index[0], "Extra Points"] = float(extra or 0.0)
...
with tab_summary:
    ...
    dsel = col[0].date_input("Pick a date", value=dt.date.today(), format="MM/DD/YYYY")
    ...
    if not day_df.empty:
        ...
        # Build per-category breakdown using minute winners
        # Recompute winners to show per-category hours/points
        wknd = dsel.weekday() >= 5
    ...
    # Monthly summary table taken from the 'Monthly Summary' sheet
    # Recompute on the fly for display
    if not entries.empty:
        per_day = []
