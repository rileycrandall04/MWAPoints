...
def write_daily_totals(sh, df_entries):
    ...
    for d, chunk in dfe.groupby("Date"):
        ...
        out_rows.append([
            d.strftime("%Y-%m-%d"), holiday_flag, time_pts, prod_pts, extra_pts, round(tee_pts, 2), round(total, 2)
        ])
    if out_rows:
...
def write_month_sheets(sh, df_entries):
    ...
    dfe["Entry Total Points"] = dfe["Entry Base Time Points"] + dfe["Prod"] + dfe["Extra"] + (
        dfe["TEE Exams"].fillna(0.0).astype(float) * 22.0
    )
...
with tab_entries:
    ...
    # One-time adders (apply to Day 1 or Day 2 when spanning midnight)
    ...
    st.caption("Intervals can span midnight or multiple days. Minutes after midnight go to the next day's totals. Overlaps award the higher-paying category per minute.")
    ...
    # Render intervals with stable IDs
    new_intervals = []
    new_ids = []
    delete_indices = set()
    ...
    for row in st.session_state.intervals_v2:
        ...
        for d, smin, emin in _split_across_midnights(start_dt, end_dt):
            ...
            preview_rows.append({
                "Date": d,
                "Holiday": False,  # set after per-day toggles
    ...
    # Per-day holiday toggles & adder placement (when two days)
    sorted_dates = sorted(affected_dates)
    holiday_map = {}
    adders_day_index = 0  # 0 => first day, 1 => second day
    ...
    if not preview_df.empty:
        ...
        # One-time adders (applied to chosen day)
        ...
        # Compensation Band Breakdown (time-based only, combined across previewed days)
