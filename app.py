...
def _parse_time_any(txt, label):
    ...
    txt = txt.replace("am","").replace("pm","")  # simple strip; AM/PM handling can be added if needed
    txt = txt.replace(":", "")
...
start_text = c[2].text_input("Start time (e.g. 730, 7:30, 0730)", "730")
end_text   = c[3].text_input("End time (e.g. 1700, 17:00)", "1700")
...
st.caption("Preview updates live. Times can be typed with or without a colon.")
