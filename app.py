...
with tab_entries:
    ...
    # Initialize multi-interval state
    ...
    # Render intervals with stable IDs and SYNC values back to state
    new_intervals = []
    new_ids = []
    delete_indices = set()

    for idx, row in enumerate(st.session_state.intervals_v2):
        ...
        sdate = c[1].date_input("Clock In Date", value=row.get("start_date", dt.date.today()), format="MM/DD/YYYY", key=f"sdate_{iid}")
        stext = c[2].text_input("Clock In Time", row.get("start_text",""), key=f"stext_{iid}")
        edate = c[3].date_input("Clock Out Date", value=row.get("end_date", sdate), format="MM/DD/YYYY", key=f"edate_{iid}")
        etext = c[4].text_input("Clock Out Time", row.get("end_text",""), key=f"etext_{iid}")
        delete = c[5].button("🗑️", key=f"del_{iid}")

        # --- CRITICAL: Sync UI values back into the interval row so parsing uses what you typed ---
        row["category"] = cat
        row["start_date"] = sdate
        row["start_text"] = stext
        row["end_date"] = edate
        row["end_text"] = etext
    ...
    for row in st.session_state.intervals_v2:
        ...
        for d, smin, emin in _split_across_midnights(start_dt, end_dt):
            ...
            preview_rows.append({
                "Date": d,
                "Holiday": False,  # set after we know which days are affected
            ...
            for m, rate in enumerate(minutes_winner):
                if rate > 0:
                    ...
                    if mult in band_minutes:
                        band_minutes[mult] += 1
                        band_points[mult] += rate / 60.0  # points/min -> add directly

        # AR-based displayed hourly rates
        AR_BASE = 20.0
    ...
    if st.button("Add to Sheet"):
        if not preview_df.empty:
            ...
            if chosen_date is not None:
                # Pick the first row for the chosen date
