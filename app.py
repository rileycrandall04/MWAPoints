...
with tab_entries:
    ...
    with st.form("add_entry_form", clear_on_submit=False):
        ...
        start_t = parse_time(start_text)
        end_text = c[3].text_input("End time (e.g. 1700, 5:00pm)", "1700")
        def parse_end(txt):
            txt = txt.replace(":", "").strip()
            if not txt.isdigit() or len(txt) < 2 or len(txt) > 4:
                st.warning("Invalid end time")
                return None
            if len(txt) <= 2:
                hh, mm = int(txt), 0
            else:
        ...
        st.caption("Select date from the calendar or type as DD/MM/YYYY. Times support minute precision.")
        ...
        preview_html += f"\n\n**Projected total for {date.strftime('%d/%m/%Y')}: {projected_daily_total:.2f} points**"
...
def entry_time_points(category: str, start_t: dt.time, end_t: dt.time):
    # Keep logic consistent with band_breakdown()
    ...
    if category in ("Assigned (General AR)", "Activation from Unrestricted Call"):
        base=20.0
        subtotal = d*base*1.00 + e*base*1.10 + n*base*1.25
        if category=="Assigned (General AR)":
            return max(subtotal, 80.0)  # 4-hour minimum
        return subtotal
