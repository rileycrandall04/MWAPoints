...
def ensure_user_sheet(gc):
    ...
    def get_or_create(name, header=None):
        try:
    ...
    ws_entries = get_or_create("Entries", header=[
        "Date","Category","Start","End","TEE Exams","Productivity Points","Extra Points","Notes"
    ])
    ws_daily = get_or_create("Daily Totals", header=[
        "Date","Time Points","Productivity Points","Extra Points","TEE Points","Total Points","Running Monthly Total"
    ])
    ws_msum = get_or_create("Monthly Summary", header=["Month","Total Points"])
    ...
    date_live = c[0].date_input("Date", value=dt.date.today(), format="MM/DD/YYYY", key="live_date")
    ...
    rows = preview_rows(date_live, category, start_t, end_t, tee, prod, extra, notes) if (start_t and end_t) else []
    ...
    if not entries.empty and start_t and end_t:
        same_day = entries[entries["Date"] == date_live]
    ...
    if rows:
        ...
        if float(extra or 0.0) != 0.0:
            st.markdown(f"**New extra points:** {float(extra):.2f}")
        st.markdown(f"**Projected total for {date_live.strftime('%d/%m/%Y')}: {projected_daily_total:.2f} points**")
    else:
