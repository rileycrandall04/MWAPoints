...
with tab_entries:
    st.subheader("Add Time Intervals (with dates)")

    # Per-session one-time adders (can be applied to Day 1 or Day 2 when spanning midnight)
    ...
    st.caption("Intervals can span midnight or multiple days. Minutes after midnight are automatically attributed to the next day's totals. Overlaps are resolved by awarding the higher-paying category per minute.")

    # Multi-interval widget with start/end dates
    if "intervals_v2" not in st.session_state:
        st.session_state.intervals_v2 = [ {
    ...
    if st.button("➕ Add interval"):
        st.session_state.intervals_v2.append({
            "category": CATEGORIES[0],
            "start_date": dt.date.today(),
            "start_text": "",
            "end_date": dt.date.today(),
...
# Ensure each interval has a stable unique ID
if "interval_ids" not in st.session_state:
    st.session_state.interval_ids = [f"int_{i}_{int(time.time()*1000)}" for i in range(len(st.session_state.intervals_v2))]
else:
    # Expand or shrink interval_ids to match
    while len(st.session_state.interval_ids) < len(st.session_state.intervals_v2):
        st.session_state.interval_ids.append(f"int_{len(st.session_state.interval_ids)}_{int(time.time()*1000)}")
    while len(st.session_state.interval_ids) > len(st.session_state.intervals_v2):
        st.session_state.interval_ids.pop()

for i, row in enumerate(st.session_state.intervals_v2):
    iid = st.session_state.interval_ids[i]

        st.markdown(f"**Interval {i+1}**")
        c = st.columns([1.8,1,1.1,1,1.1,0.3])
        cat = c[0].selectbox("Category", CATEGORIES, index=CATEGORIES.index(row["category"]) if row["category"] in CATEGORIES else 0, key=f"cat2_{i}")
        sdate = c[1].date_input("Clock In Date", value=row.get("start_date", dt.date.today()), format="MM/DD/YYYY", key=f"sdate_{i}")
        stext = c[2].text_input("Clock In Time", row.get("start_text",""), key=f"stext_{i}")
        edate = c[3].date_input("Clock Out Date", value=row.get("end_date", sdate), format="MM/DD/YYYY", key=f"edate_{i}")
        etext = c[4].text_input("Clock Out Time", row.get("end_text",""), key=f"etext_{i}")
        delete = c[5].button("🗑️", key=f"del2_{i}")
        if not delete:
            new_intervals.append({
                "category": cat, "start_date": sdate, "start_text": stext,
                "end_date": edate, "end_text": etext
            })
    st.session_state.intervals_v2 = new_intervals

    # Build preview rows: split across midnights into per-date slices
    preview_rows = []
    affected_dates = set()

    for row in st.session_state.intervals_v2:
        stime = parse_time_any(row["start_text"])
        etime = parse_time_any(row["end_text"])
        sdate = row["start_date"]; edate = row["end_date"]
        if not (stime and etime and isinstance(sdate, dt.date) and isinstance(edate, dt.date)):
            continue
        if edate == sdate and (to_minutes(etime) <= to_minutes(stime)):
        ...
        for d, smin, emin in _split_across_midnights(start_dt, end_dt):
            ...
            if emin <= smin:
                continue
            end_t = minutes_to_time(emin-1) if emin == 1440 else minutes_to_time(emin)
            preview_rows.append({
                "Date": d,
                "Holiday": False,  # we'll set per-day holidays after we know which days are affected
    ...
    if len(sorted_dates) == 0:
        ...
        # Apply holiday flags to preview_df
    ...
    if not preview_df.empty:
        ...
        # ---- Compensation Band Breakdown (time-based only, combined across all previewed days) ----
        st.markdown("### Compensation Band Breakdown (time-based only)")
            ...
            for m, rate in enumerate(minutes_winner):
                if rate > 0:
                    ...
                    if mult in band_minutes:
                        band_minutes[mult] += 1
                        band_points[mult] += rate / 60.0  # already points/min

        # AR-based displayed hourly rates (per your request)
        AR_BASE = 20.0
        display_rate = {1.00: AR_BASE*1.00, 1.10: AR_BASE*1.10, 1.25: AR_BASE*1.25}
        ...
        for mult in [1.00, 1.10, 1.25]:
            ...
            rows.append({
                "Band": f"{mult:.2f}×",
                "Rate Used": f"{display_rate[mult]:.2f} pts/hr",
                "Hours": round(hrs, 2),
                "Points": round(pts, 2),
            })
        ...
        band_df = pd.DataFrame(rows, columns=["Band","Rate Used","Hours","Points"])
    ...
    if st.button("Add to Sheet"):
        if not preview_df.empty:
            ...
            chosen_date = sorted_dates[min(adders_day_index, len(sorted_dates)-1)] if len(sorted_dates)>0 else None
